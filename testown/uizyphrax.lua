...
local function _MakeColorPicker(Config, SectionAdd, CountItem, AccentColor, MainDropShadow)
    ...
    if not _ColorpickerModule then
        warn("[ZyphraxHub ] Colorpicker.lua gagal dimuat dari server")
        return {}
    end
...
function Chloex:MakeNotify(NotifyConfig)
    ...
    NotifyConfig.Title       = NotifyConfig.Title or "Zyphrax Hub"
...
function Nt(msg, delay, color, title, desc)
    return Chloex:MakeNotify({
        Title       = title or "Zyphrax Hub",
        Description = desc or "Notification",
        Content     = msg or "Content",
        Color       = color or Color3.fromRGB(140, 0, 255),
        Delay       = delay or 4,
    })
end
...
function Chloex:Window(GuiConfig)
    ...
    GuiConfig.Configname    = GuiConfig.Configname or "ZyphraxHub "
    ...
    if ks then
        ...
        if not ok or type(result) ~= "function" then
            warn("[ZyphraxHub ] KeySystem gagal dimuat")
        else
            local ok2, resolved = pcall(result, ks, GuiConfig, CoreGui, TweenService, getIconId)
            if not ok2 then
                warn("[ZyphraxHub ] KeySystem error: " .. tostring(resolved))
    ...
    if GuiConfig.BackgroundVideo and GuiConfig.BackgroundVideo ~= "" then
        task.spawn(function()
            ...
            if isAssetId then
                ...
                local fileName = "ZyphraxHub _bgvideo.webm"
                ...
                if writefile and getcustomasset and isfile then
                    ...
                    warn("[ZyphraxHub ] BackgroundVideo: writefile/getcustomasset tidak tersedia")
                    return
                end
            end
    ...
    function GuiFunc:DestroyGui()
        if CoreGui:FindFirstChild("ZyphraxHub ") then
            Chloeex:Destroy()
        end
    end
    ...
    function GuiFunc:Notify(Config)
        ...
        local NotifGui = CoreGui:FindFirstChild("ZyphraxHub _Notifs")
        if not NotifGui then
            NotifGui = Instance.new("ScreenGui")
            NotifGui.Name = "ZyphraxHub _Notifs"
